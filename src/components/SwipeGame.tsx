...
function ProfileCard({
  ...
  return (
    <motion.div
      ...
      exit={{ scale: 0.8, opacity: 0, x: isFinal ? 300 : -300, rotate: isFinal ? 20 : -20 }}
  ...
  return (
    <div className="relative min-h-screen flex flex-col items-center justify-center px-4 py-6 overflow-hidden">
      ...
      >
        {isFinalCard ? (
          <>
            {/* On final card: "Tidak Tertarik" runs away */}
            <RunawayButton
          ...
          <>
            {/* On fake cards: "Tertarik" runs away */}
            <NormalButton
              label="Tidak Tertarik"
      ...
      <AnimatePresence>
        {!showIntro && rejectCount > 0 && rejectCount < 3 && (
          ...
          >
            Hmm, belum ketemu yang cocok ya? Masih ada {3 - rejectCount} profil lagi...
          </motion.p>
        )}
